...
// Declaración adelantada de las rutinas de tratamiento de
...
int uart_lconf(enum UART port, struct ulconf *lconf)
{
	...
	//Word length
	if(lconf->wordlen == EIGHT)
		confvalue = 3;
	else if(lconf->wordlen == SEVEN)
		confvalue = 2;
	else if(lconf->wordlen == SIX)
		confvalue = 1;
...
int uart_conf_txmode(enum UART port, enum URxTxMode mode)
{
	...
	switch (port) {
		case UART0:
			rUCON0 |= (conf << 2);//COMPLETAR: modo indicado por conf, Tx interrupt por nivel
			rUCON0 |= (1 << 9);
			break;

		case UART1:
			rUCON1 |= (conf << 2);//COMPLETAR: modo indicado por conf, Tx interrupt por nivel
			rUCON1 |= (1 << 9);
			break;
	}
...
int uart_conf_rxmode(enum UART port, enum URxTxMode mode)
{
	...
	switch (port) {
		case UART0:
			rUCON0 |= conf;//COMPLETAR: modo indicado por conf, Rx interrupt por pulso
			rUCON0 &= ~(0x1 << 8);
		...
		case UART1:
			rUCON1 |= conf; //COMPLETAR: modo indicado por conf, Rx interrupt por pulso
			rUCON1 &= ~(0x1 << 8);
			//COMPLETAR: si se el modo es por interrupciones habilitar la línea
...
/* uart_rx_ready: función que realiza una espera activa hasta que el puerto haya
 * recibido un byte
 */
