...
int tmr_set_prescaler(int p, int  value)
{
	...
	rTCFG0 |= (0xFF << offset);		// Poner a 1 los bits correspondientes a p.
	rTCFG0 &= (value << offset);	// Poner en los bits correspondientes a p el valor value.
...
int tmr_set_divider(int d, enum tmr_div div)
{
	...
	rTCFG1 &= ~(0xF << pos);		// Poner a 1 los bits correspondientes a d.
	rTCFG1 |= (div << pos);		// Poner en los bits correspondientes a d el valor de div.
